...
int main(int argc, char **argv) {
	...
	for(int i = 0; i < 100; i++) {
		flock.push_back(Boid(
			Vec2<float>(rand() % WINDOW_WIDTH, rand() % WINDOW_HEIGHT),
			Vec2<float>(rand() % 250 - 250, rand() % 250 - 250),
			300, 
			30,
			i
		));
	...
	while(!quit) {
		...
		for(auto &boid : flock) {
			Vec2<float> top(boid.pos.x, boid.pos.y + Boid::size * 4),
			...
			Vec2<float>::rotate(top, boid.pos, angle);
			Vec2<float>::rotate(bottomLeft, boid.pos, angle);
			Vec2<float>::rotate(bottomRight, boid.pos, angle);
